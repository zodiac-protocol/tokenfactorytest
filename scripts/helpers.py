...
def init_contract(code_id, init_msg, wallet, terra, name, coins=None, fee_in=None):

  fee = Fee(25000000, "10000000uosmo")
  if ("terra" in terra.chain_id) or ("pisco" in terra.chain_id):
...
def bytes_to_hexstring(bytes):
  return b64_to_hexstring(bytes_to_b64(bytes))

def bytes_to_bech32(prefix, bytes):
  return b64_to_bech32(prefix, base64.b64encode(bytes))
...
def fetch_chain_objects(chain_id):
  ...
  if chain_id == "pisco-1":
  ...
  elif chain_id == "atlantic-2":
    sei = LCDClient(url="https://rest.atlantic-2.seinetwork.io", chain_id="localterra")
    sei.chain_id = "atlantic-2"
    sei.broadcaster = BaseAPI2(sei)

    sei_rpc_url = "https://rpc-2.seinetwork.io"
    sei_rpc_header = {}

    wallet = sei.wallet(OsmoKey(mnemonic=seed_phrase, coin_type=118))
    return (sei, wallet, sei_rpc_url, sei_rpc_header)
    ...
    terra = LCDClient(url="http://xxx", chain_id="localterra")
    terra.broadcaster = BaseAPI2(terra)

    #terra_rpc_url = f"https://rpc.pisco.terra.setten.io/{creds['setten_project_id']}"
    #terra_rpc_header = {"Authorization": f"Bearer {creds['setten_key']}"}
    terra_rpc_url = "http://xxx"
    terra_rpc_header = {}
    
    wallet = terra.wallet(MnemonicKey(mnemonic=seed_phrase))

    return (terra, wallet, terra_rpc_url, terra_rpc_header)
  elif chain_id == "localosmosis":
    osmo = LCDClient(url="http://xxx", chain_id="localterra")
    osmo.chain_id = "localosmosis"
    osmo.broadcaster = BaseAPI2(osmo)

    osmo_rpc_url = "https://xxx"
    osmo_rpc_header = {}

    wallet = osmo.wallet(OsmoKey(mnemonic=seed_phrase, coin_type=118))
    return (osmo, wallet, osmo_rpc_url, osmo_rpc_header)

  elif chain_id == "edgenet":
    osmo = LCDClient(url="https://lcd.edgenet.osmosis.zone", chain_id="localterra")
    osmo.chain_id = "edgenet"
    osmo.broadcaster = BaseAPI2(osmo)

    osmo_rpc_url = "https://rpc.edgenet.osmosis.zone"
    osmo_rpc_header = {}
...
def create_osmo_chain_objects(lcd_url, chain_id):
  creds = {}
  # Open the file for reading
  with open("/repos/pybc-relayer/scripts/creds.json", "r") as f:
    # Load the dictionary from the file
  ...
  client = LCDClient(f"http://{lcd_url}", chain_id="localterra")
  client.chain_id = chain_id
  client.broadcaster = BaseAPI2(client)

  rpc_url = f"https://{lcd_url}:26657"
  rpc_header = {}

  wallet = client.wallet(OsmoKey(mnemonic=seed_phrase, coin_type=118))

  return (client, wallet, rpc_url, rpc_header)
...
def fetch_packet_proof(rpc_url, rpc_header, trusted_height, trusted_revision_number, packet_row, port_id, channel_id):
  ...
  params = {
    "path": '"/store/ibc/key"',
    "data": "0x" + bytes(f"commitments/ports/{port_id}/channels/{channel_id}/sequences/{int(packet_row['packet_sequence'])}", "ascii").hex(),
    "prove": "true",
    "height": int(trusted_height) - 1,
  }
  resp = requests.get(f"{rpc_url}/abci_query", headers=rpc_header, params=params).json()
...
def cleanup_wallet(wallet, dump_wallet, osmo):
  for c in osmo.bank.balance(wallet.key.acc_address)[0].to_list():
    if c.denom not in ["uion", "uosmo"]:
      bank_msg_send(dump_wallet.key.acc_address, Coins([Coin(denom=c.denom, amount=c.amount)]), wallet, osmo)